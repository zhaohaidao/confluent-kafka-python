...
BROKERS="${SOAK_BOOTSTRAP_SERVERS:-10.13.10.79:9092,10.32.12.69:9092,10.13.2.94:9092}"
TOPIC="${SOAK_TOPIC:-mcft_topic_p10}"
RATE="${SOAK_RATE:-20}"
DURATION_SECONDS="${SOAK_DURATION_SECONDS:-0}"
MAX_NO_PROGRESS_SECONDS="${SOAK_MAX_NO_PROGRESS_SECONDS:-315360000}"
...
MESSAGE_PREFIX="${SOAK_MESSAGE_PREFIX:-red-soak-lineage}"
MESSAGE_MARKER="${SOAK_MESSAGE_MARKER:-}"
GROUP="${SOAK_GROUP:-}"
OFFSET_RESET="${SOAK_OFFSET_RESET:-latest}"
METRICS_ENV_CONFIG="${SOAK_METRICS_ENV_CONFIG:-prod}"
METRICS_COLLECT_URL="${SOAK_METRICS_COLLECT_URL:-}"
...
LIBRDKAFKA_LIB_PATH="${LIBRDKAFKA_LIB_PATH:-/home/admin/mh/kafka/librdkafka/.install-auth-redfetch-current/lib}"
export LD_LIBRARY_PATH="${LIBRDKAFKA_LIB_PATH}:${LD_LIBRARY_PATH}"
...
cmd=(
    ...
    --metrics-env-config "${METRICS_ENV_CONFIG}"
)
...
if [[ -n "${METRICS_COLLECT_URL}" ]]; then
    cmd+=(--metrics-collect-url "${METRICS_COLLECT_URL}")
fi
