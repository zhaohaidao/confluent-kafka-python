...
class MetricsHelper:
    ...
    def parse_stats_for_metrics(stats, dest):
        ...
        for topic in topics_list:
            ...
            # Partition stats dominate payload size and are not consumed by the
            # fragmentation dashboard this sender targets.
...
class MetricsCollector:
    ...
    def _get_config_metrics(self):
        ...
        for key, val in config_source.items():
            ...
            if self._is_sensitive_config_key(normalized_key):
                metrics[normalized_key] = _REDACTED_CONFIG_VALUE
                continue
            if isinstance(val, bool):
                ...
                val_str = str(val)
            if "0x" in val_str:
                # librdkafka config dumps include pointer-like opaque values;
                # they are process-local noise and should not leave the process.
                continue
    ...
    def _is_sensitive_config_key(self, key):
