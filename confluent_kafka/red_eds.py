...
def resolve_bootstrap(conf):
    ...
    if route is None:
        _LOGGER.info(
            "bootstrap resolution skipped: route=plain original=%s config=%s",
            conf.get(BOOTSTRAP_SERVERS_CONFIG),
    ...
    unique_addresses = _unique_preserve_order(addresses)
    bootstrap_servers = ",".join(unique_addresses)
    resolved = dict(conf)
    ...
    resolved[BOOTSTRAP_SERVERS_CONFIG] = bootstrap_servers
    if route_type == "eds":
        _LOGGER.info(
            "EDS bootstrap route selected: original_eds=%s original_service=%s "
            "queried_service=%s address=%s",
            original_bootstrap,
            logical_name,
            EDS_BOOTSTRAP_PREFIX + resolved_logical_name,
            bootstrap_servers,
        )
    else:
        _LOGGER.info(
            "bootstrap route selected: route=%s original=%s logical=%s address=%s",
            route_type,
            original_bootstrap,
            resolved_logical_name,
            bootstrap_servers,
