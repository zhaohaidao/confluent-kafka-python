...
RUN set -eux; \
    ...
    shared_linker_flags=""; \
    if [ "${MANYLINUX_BASELINE}" != "2_28" ]; then \
        shared_linker_flags="-Wl,-Bsymbolic-functions"; \
    fi; \
    ...
    if [ "${MANYLINUX_BASELINE}" = "2_28" ]; then \
        ...
        if grep -Eq '(^|[[:space:]])T[[:space:]]+thrd_create$' /tmp/librdkafka-symbols.txt; then \
            echo "librdkafka must use glibc C11 thrd_create for manylinux_2_28" >&2; \
            exit 1; \
        fi; \
    else \
        if grep -Eq 'U[[:space:]]+thrd_create(@|$)' /tmp/librdkafka-symbols.txt; then \
            echo "manylinux2014 librdkafka must not call host glibc thrd_create" >&2; \
            exit 1; \
        fi; \
        if objdump -R "${libpath}" | grep -q 'thrd_create'; then \
            echo "manylinux2014 librdkafka has an interposable thrd_create relocation" >&2; \
            exit 1; \
        fi; \
        grep -Eq '(^|[[:space:]])T[[:space:]]+thrd_create$' /tmp/librdkafka-symbols.txt; \
    fi; \
...
RUN set -eux; \
    ...
    if [ "${MANYLINUX_BASELINE}" = "2_28" ]; then \
        ...
        grep -Eq '(^|[[:space:]])T[[:space:]]+thrd_create$' /tmp/librdkafka-symbols.txt; \
        if objdump -R "${LIBRDKAFKA_PREFIX}/lib/librdkafka.so" | grep -q 'thrd_create'; then \
            echo "manylinux2014 librdkafka has an interposable thrd_create relocation" >&2; \
            exit 1; \
        fi; \
    fi; \
