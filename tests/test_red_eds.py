...
def test_resolve_eds_bootstrap(monkeypatch, caplog):
    ...
    assert "EDS bootstrap route selected" in caplog.text
    assert "original_eds=eds://kafka-eds-paastest" in caplog.text
    assert "original_service=kafka-eds-paastest" in caplog.text
    assert "queried_service=eds://kafka-eds-paastest" in caplog.text
    assert "address=10.0.0.1:9092,10.0.0.2:9092" in caplog.text
...
def test_resolve_eds_logs_route_and_redacts_sensitive_config(monkeypatch, caplog):
    ...
    assert "original_eds=eds://kafka-eds-paastest" in caplog.text
    assert "original_service=kafka-eds-paastest" in caplog.text
    assert "queried_service=eds://kafka-eds-paastest-sasl" in caplog.text
    assert "address=10.1.1.1:9093" in caplog.text
...
def test_resolve_cluster_bootstrap(monkeypatch, caplog):
    ...
    assert "EDS bootstrap route selected" not in caplog.text
    assert "bootstrap route selected: route=cluster" in caplog.text
    assert "original=cluster://kafka-main" in caplog.text
    assert "logical=kafka-main" in caplog.text
    assert "address=10.0.0.2:9092,10.0.0.1:9092" in caplog.text
...
def test_plain_bootstrap_logs_skipped_resolution(caplog):
    caplog.set_level(logging.INFO, logger="confluent_kafka.red_eds")
    ...
    assert "bootstrap resolution skipped: route=plain" in caplog.text
    assert "original=10.0.0.1:9092" in caplog.text
    assert "EDS bootstrap route selected" not in caplog.text
    assert "bootstrap route selected: route=" not in caplog.text
